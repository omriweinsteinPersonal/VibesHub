...
describe('storefront text blocks', () => {
  it('distinguishes unrelated updates from explicitly removing all text', () => {
    expect(
      creatorStorefrontConfigurationInputSchema.parse({ categoryIds: [] }).titles,
    ).toBeUndefined();
    expect(
      creatorStorefrontConfigurationInputSchema.parse({ categoryIds: [], titles: [] })
        .titles,
...
describe('storefront block editor', () => {
  it('keeps unrelated writes separate from resetting the brand order', () => {
    expect(
      creatorStorefrontConfigurationInputSchema.parse({ categoryIds: [] }).brandOrder,
    ).toBeUndefined();
    expect(
      creatorStorefrontConfigurationInputSchema.parse({ categoryIds: [], brandOrder: [] })
        .brandOrder,
  ...
  it('round trips a text card and rejects unsafe CSS values and duplicate brands', () => {
    ...
    expect(
      creatorStorefrontConfigurationInputSchema.safeParse({
        categoryIds: [],
        titles: [{ ...text, background: 'url(https://example.com)' }],
      }).success,
    ).toBe(false);
    expect(
      creatorStorefrontConfigurationInputSchema.safeParse({
